...
makeObjGuard(T_* t)
{
    mCHECK(!(std::is_base_of<T, T_>::value || std::is_same<T, T_>::value));
...
{
    void* deviceMem;
    mCHECK(cudaMalloc(&deviceMem, memSize));
...
public:
    GpuTimer(cudaStream_t stream)
        : mStream(stream)
    {
        mCHECK(cudaEventCreate(&mStart));
        mCHECK(cudaEventCreate(&mStop));
    }
    ~GpuTimer()
    {
        mCHECK(cudaEventDestroy(mStart));
        mCHECK(cudaEventDestroy(mStop));
    }
    void start()
    {
        mCHECK(cudaEventRecord(mStart, mStream));
    }
    void stop()
    {
        mCHECK(cudaEventRecord(mStop, mStream));
        float ms{0.0f};
        mCHECK(cudaEventSynchronize(mStop));
        mCHECK(cudaEventElapsedTime(&ms, mStart, mStop));
        mMs += ms;
    }
...
inline int32_t getSMVersion()
{
    ...
    mCHECK(cudaGetDevice(&deviceIndex));

    int32_t major, minor;
    mCHECK(cudaDeviceGetAttribute(&major, cudaDevAttrComputeCapabilityMajor, deviceIndex));
    mCHECK(cudaDeviceGetAttribute(&minor, cudaDevAttrComputeCapabilityMinor, deviceIndex));
